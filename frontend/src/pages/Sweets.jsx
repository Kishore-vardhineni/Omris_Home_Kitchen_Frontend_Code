...
const Sweets = () => {
  ...
  const fetchProducts = async () => {
    ...
    setError(null);
    try {
      const data = await getProducts({ category: 'sweet' });
      setProducts(data.products || []);
    } catch (err) {
      setError(err.message || 'Failed to load sweets. Please try again.');
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="tomato-section">
          ...
          {loading && (
            <div className="flex flex-col items-center justify-center py-20 gap-4">
              ...
              <p className="text-[var(--text-dark)] font-medium">Fetching fresh sweets...</p>
            </div>
          )}
          ...
          {!loading && !error && products.length === 0 && (
            <div className="text-center py-20 bg-[var(--bg-cream)] rounded-xl border border-dashed border-[var(--primary-dark-green)] border-opacity-20 max-w-lg mx-auto">
              ...
              <p className="text-[var(--text-dark)] opacity-80 mb-6">We don't have any Sweets in stock right now. Check back soon!</p>
            </div>
          )}
